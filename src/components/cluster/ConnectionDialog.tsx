...
const KAFKA_VERSIONS = ['3.7', '3.6', '3.5', '3.4', '3.3', '3.2', '3.1', '3.0', '2.8', '2.7', '2.6', 'custom'];
...
const DEFAULT_FORM: ConnectionFormData = {
  name: '', bootstrapServers: '', kafkaVersion: '3.7',
  zookeeperHost: '', zookeeperPort: 2181, zkChrootPath: '/',
  securityProtocol: 'PLAINTEXT', saslMechanism: '', jaasConfig: '',
  awsRegion: '', sslCaCertPath: '', sslClientCertPath: '',
  sslClientKeyPath: '', sslClientKeyPassword: '', sslVerifyHostname: true,
  schemaRegistryUrl: '', schemaRegistryUsername: '', schemaRegistryPassword: '',
  connectWorkerUrls: '', notes: '',
};
    ...
    <div style={{ position: 'relative' }}>
      ...
      <button
        ...
        style={{
          ...
          color: 'var(--color-text-faint)', padding: 2,
...
function FilePickerInput({ value, onChange, label }: { value: string; onChange: (v: string) => void; label: string }) {
  const t = useT();
  return (
    ...
    >
      ...
      >
        ...
        <div style={{
          ...
          <h2 style={{ fontFamily: 'var(--font-heading)', fontSize: 15, fontWeight: 600, color: 'var(--color-text)' }}>
          ...
          <button
            ...
            style={{
              background: 'none', border: 'none', cursor: 'pointer',
              color: 'var(--color-text-faint)', padding: 4, borderRadius: 'var(--radius-sm)',
        ...
        <div role="tablist" style={{
          ...
          {(['basic', 'security', 'advanced'] as Tab[]).map((tabKey) => (
            <button
              ...
              onMouseEnter={e => { if (tab !== tabKey) (e.currentTarget.style.color = 'var(--color-text)'); }}
              onMouseLeave={e => { if (tab !== tabKey) (e.currentTarget.style.color = 'var(--color-text-muted)'); }}
            >
              {TAB_LABELS[tabKey]}
            </button>
          ))}
            ...
            <div>
              <Field label={t('conn.nameLabel')} required>
                <Input
                  ...
                  maxLength={64}
                />
                {errors.name && <p style={{ color: 'var(--color-error)', fontSize: 11, marginTop: 3 }}>{errors.name}</p>}
              </Field>
              ...
              <Field label={t('conn.bootstrapServers')} required>
                ...
                />
                {errors.bootstrapServers && <p style={{ color: 'var(--color-error)', fontSize: 11, marginTop: 3 }}>{errors.bootstrapServers}</p>}
              </Field>

              <Field label={t('conn.kafkaVersion')} required>
                <Select value={form.kafkaVersion} onChange={v => set('kafkaVersion', v)}>
                  {KAFKA_VERSIONS.map(v => <option key={v} value={v}>{v}</option>)}
                </Select>
              </Field>

              <div style={{ display: 'grid', gridTemplateColumns: '1fr 100px', gap: 10 }}>
                <Field label={t('conn.zookeeperAddr')}>
                  <Input value={form.zookeeperHost} onChange={e => set('zookeeperHost', e.target.value)} placeholder={t('conn.zookeeperPlaceholder')} />
                </Field>
                <Field label={t('conn.port')}>
                  <Input type="number" value={form.zookeeperPort} onChange={e => set('zookeeperPort', Number(e.target.value))} />
                </Field>
              </div>

              <Field label={t('conn.chrootPath')}>
                <Input value={form.zkChrootPath} onChange={e => set('zkChrootPath', e.target.value)} placeholder="/" fontFamily="var(--font-heading)" />
              </Field>
            </div>
          )}
          ...
          {tab === 'security' && (
            <div>
              <Field label={t('conn.securityProtocol')} required>
                <Select value={form.securityProtocol} onChange={v => set('securityProtocol', v as SecurityProtocol)}>
                  {(['PLAINTEXT', 'SASL_PLAINTEXT', 'SSL', 'SASL_SSL'] as SecurityProtocol[]).map(p => (
              ...
              {hasSasl && (
                <>
                  <Field label={t('conn.saslMechanism')} required>
                    <Select value={form.saslMechanism} onChange={v => set('saslMechanism', v as SaslMechanism)}>
                      <option value="">{t('conn.saslPlaceholder')}</option>
                      {(['PLAIN', 'SCRAM-SHA-256', 'SCRAM-SHA-512', 'GSSAPI', 'AWS_MSK_IAM'] as SaslMechanism[]).map(m => (
                  ...
                  {form.saslMechanism === 'AWS_MSK_IAM' && (
                    <Field label={t('conn.awsRegion')}>
                      <Input value={form.awsRegion} onChange={e => set('awsRegion', e.target.value)} placeholder="us-east-1" />
                    </Field>
                  )}
                </>
              )}
              ...
              {hasSsl && (
                <>
                  <Field label={t('conn.caCertPath')} required>
                    <FilePickerInput value={form.sslCaCertPath} onChange={v => set('sslCaCertPath', v)} label={t('conn.caCert')} />
                  </Field>
                  <Field label={t('conn.clientCertPath')}>
                    <FilePickerInput value={form.sslClientCertPath} onChange={v => set('sslClientCertPath', v)} label={t('conn.clientCert')} />
                  </Field>
                  <Field label={t('conn.clientKeyPath')}>
                    <FilePickerInput value={form.sslClientKeyPath} onChange={v => set('sslClientKeyPath', v)} label={t('conn.clientKey')} />
                  </Field>
                  <Field label={t('conn.keyPassword')}>
                    <PasswordInput value={form.sslClientKeyPassword} onChange={v => set('sslClientKeyPassword', v)} />
                  </Field>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 14 }}>
                    ...
                    />
                    <label htmlFor="sslVerify" style={{ ...labelStyle, marginBottom: 0, cursor: 'pointer' }}>{t('conn.sslVerifyHostname')}</label>
                  </div>
                </>
              )}

              {form.securityProtocol === 'PLAINTEXT' && (
                <p style={{ color: 'var(--color-text-faint)', fontSize: 12, textAlign: 'center', padding: '20px 0' }}>
          ...
          {tab === 'advanced' && (
            <div>
              <Field label={t('conn.schemaRegistryUrl')}>
                <Input value={form.schemaRegistryUrl} onChange={e => set('schemaRegistryUrl', e.target.value)} placeholder="http://localhost:8081" fontFamily="var(--font-heading)" />
              </Field>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                <Field label={t('conn.schemaRegistryUsername')}>
                  <Input value={form.schemaRegistryUsername} onChange={e => set('schemaRegistryUsername', e.target.value)} />
                </Field>
                <Field label={t('conn.schemaRegistryPassword')}>
                  <PasswordInput value={form.schemaRegistryPassword} onChange={v => set('schemaRegistryPassword', v)} />
                </Field>
              </div>
              <Field label={t('conn.connectWorkerUrls')}>
                <textarea
                  ...
                  style={{ ...inputStyle, resize: 'vertical', fontFamily: 'var(--font-heading)', fontSize: 12, lineHeight: 1.6 }}
                  onFocus={e => (e.currentTarget.style.borderColor = 'var(--color-primary)')}
                  onBlur={e => (e.currentTarget.style.borderColor = 'var(--color-border)')}
                />
              </Field>
              <Field label={t('conn.notes')}>
                <textarea
                  ...
                  onBlur={e => (e.currentTarget.style.borderColor = 'var(--color-border)')}
                />
                <p style={{ textAlign: 'right', fontSize: 11, color: 'var(--color-text-faint)', marginTop: 3 }}>
          ...
          <div style={{
            ...
            {testResult.success
              ? <span style={{ color: 'var(--color-success)' }}>
                  {t('conn.connectionSuccess')} · {t('conn.brokerCount', { count: testResult.brokerCount ?? 0 })} · {t('conn.topicCount', { count: testResult.topicCount ?? 0 })} · {testResult.kafkaVersion} · {testResult.latencyMs}ms
                </span>
          ...
          <div style={{ position: 'relative' }}>
                ...
                {Object.keys(CLOUD_TEMPLATES).map(name => (
                  <button
                    ...
                    style={{
                      ...
                      color: 'var(--color-text)', fontFamily: 'var(--font-body)', fontSize: 13,
                      cursor: 'pointer',
                      transition: 'background var(--transition-fast)',
                    }}
            ...
            {showTemplateConfirm && showTemplateConfirm !== 'menu' && (
              <div style={{
                ...
                background: 'var(--color-surface)', border: '1px solid var(--color-border)',
                borderRadius: 'var(--radius-md)', boxShadow: '0 8px 24px rgba(0,0,0,0.5)',
                padding: 14, width: 280, zIndex: 100,
              }}>
                <p style={{ fontSize: 13, color: 'var(--color-text)', marginBottom: 12 }}>
                  {t('conn.templateConfirm', { name: showTemplateConfirm })}
                </p>
                <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
                  <button type="button" onClick={() => setShowTemplateConfirm(null)}
                    style={{ padding: '5px 12px', background: 'var(--color-surface-2)', color: 'var(--color-text-muted)', border: '1px solid var(--color-border)', borderRadius: 'var(--radius-sm)', cursor: 'pointer', fontSize: 12, fontFamily: 'var(--font-body)' }}>
                    {t('common.cancel')}
                  </button>
                  <button type="button" onClick={() => applyTemplate(showTemplateConfirm)}
                    style={{ padding: '5px 12px', background: 'var(--color-primary)', color: 'var(--color-primary-text)', border: 'none', borderRadius: 'var(--radius-sm)', cursor: 'pointer', fontSize: 12, fontWeight: 600, fontFamily: 'var(--font-body)' }}>
