...
pub async fn describe_consumer_group(
    ...
    tokio::task::spawn_blocking(move || {
        let mut cfg = create_kafka_config(&conn);
        cfg.set("group.id", format!("kafka-desktop-manager-describe-{}", &group_id));

        let consumer: BaseConsumer = cfg.create().map_err(|e| e.to_string())?;

        // Fetch group members via group list
        let members_json: Vec<Value> = match consumer.fetch_group_list(Some(&group_id), KAFKA_RPC_TIMEOUT) {
            Ok(list) => {
                list.groups().iter().flat_map(|g| {
                    let raw = g as *const GroupInfo as *const rdkafka_sys::rd_kafka_group_info;
                    let member_cnt = unsafe {
                        let info = &*raw;
                        if info.members.is_null() { 0 } else { info.member_cnt as usize }
                    };
                    if member_cnt == 0 {
                        return vec![];
                    }
        ...
        let md = consumer.fetch_metadata(None, KAFKA_RPC_TIMEOUT).map_err(|e| e.to_string())?;
        ...
        for elem in committed.elements() {
            if let Offset::Offset(committed_off) = elem.offset() {
                ...
                let lag = if high > committed_off { high - committed_off } else { 0 };
...
pub async fn preview_reset_offsets(
    ...
    tokio::task::spawn_blocking(move || {
        let mut cfg = create_kafka_config(&conn);
        cfg.set("group.id", format!("kafka-desktop-manager-preview-{}", &group_id));
        let consumer: BaseConsumer = cfg.create().map_err(|e| e.to_string())?;
        ...
        group_cfg.set("enable.auto.commit", "false");
        let group_consumer: BaseConsumer = group_cfg.create().map_err(|e| e.to_string())?;
        let committed = group_consumer.committed_offsets(tpl, KAFKA_RPC_TIMEOUT).unwrap_or_default();

        let mut committed_map: std::collections::HashMap<i32, i64> = std::collections::HashMap::new();
        for elem in committed.elements() {
...
pub async fn reset_consumer_group_offsets(
    ...
    tokio::task::spawn_blocking(move || {
        ...
        consumer_cfg.set("group.id", format!("kafka-desktop-manager-reset-{}", &group_id));
        ...
        let offsets: Vec<(i32, i64)> = resolved.iter().map(|&(p, _, target, _)| (p, target)).collect();

        log::info!(
            "AlterConsumerGroupOffsets: group={}, topic={}, partitions={:?}",
            group_id, topic, offsets
        );
        ...
        unsafe {
            ...
            for &(p, off) in &offsets {
                let entry = rdkafka_sys::rd_kafka_topic_partition_list_add(c_tpl, c_topic.as_ptr(), p);
                (*entry).offset = off;
            }

            let c_group = CString::new(group_id.as_str()).map_err(|e| e.to_string())?;
            let alter_req = rdkafka_sys::rd_kafka_AlterConsumerGroupOffsets_new(
                c_group.as_ptr(),
                c_tpl,
            );
            ...
            rdkafka_sys::rd_kafka_AdminOptions_set_request_timeout(
                opts, timeout_ms, err_buf.as_mut_ptr(), err_buf.len(),
            );
            ...
            let result = if event.is_null() {
                ...
                if event_err as i32 != 0 {
                    let err_str = std::ffi::CStr::from_ptr(rdkafka_sys::rd_kafka_event_error_string(event))
                        .to_string_lossy()
                        .to_string();
                    ...
                    if res.is_null() {
                        ...
                        let mut cnt: usize = 0;
                        let groups = rdkafka_sys::rd_kafka_AlterConsumerGroupOffsets_result_groups(res, &mut cnt);
